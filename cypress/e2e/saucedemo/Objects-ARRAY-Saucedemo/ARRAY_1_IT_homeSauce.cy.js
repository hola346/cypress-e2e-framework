...
const tests = require ('../../../fixtures/Data-Driven Test/Multi_test.json') // similar to Fixture, keeps json in constant
...
describe('Multi IT Home Saucedemo', ()=>{
    ...
    tests.forEach(test=>{  // For each group of data {}, in this case, 4 times
        
        it('General IT for ALL', function(){
                home_sauce_page.typeUsername(test.User);
                home_sauce_page.typePassword(test.Pass);
                home_sauce_page.PressLoginButton();
                //Inventory_page_saucedemo.elements.TitleSpan().should('have.text', 'Products')

                if(test.testname == "Login_OK"){
                    Inventory_page_saucedemo.elements.TitleSpan().should('have.text', test.Expect) 
                    // User goes to Inv page from Home as Login was successful
                }
                /*
                else if(test.testname == "Login_Blocked_user"){
                    Error_Messages_Login.elements.ErrorMessage().should('contains.text', test.Expect)
                }
                else if(test.testname == "Login_Wrong user"){
                    Error_Messages_Login.elements.ErrorMessage().should('contains.text', test.Expect)
                }
